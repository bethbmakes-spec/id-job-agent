"""
Sends the job digest via email (SMTP). Fill in .env with your credentials.
Works with Gmail (use an App Password), Outlook, or any SMTP relay.
"""
import os
import smtplib
...
def send_digest(rows, csv_path):
    ...
    body = MIMEText(rows_to_html(rows), "html")
    msg.attach(body)
